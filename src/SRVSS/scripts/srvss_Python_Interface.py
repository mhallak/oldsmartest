...
class ScenarioLauncher:
	...
	node = ROSNode("gazebo_ros","gzserver",name="gazebo_srver" ,args=arguments ,output="screen" , respawn="false")     	  
    ...
    def launch_platform_controls(self):
	rospack = rospkg.RosPack()
	srvss_bobcat_pkg_path = rospack.get_path('srvss_bobcat')
	...
	node = ROSNode("controller_manager", "spawner",name="platform_controller_spawner" ,namespace="/srvss_bobcat", output="screen", respawn="true" ,
	...
	node = ROSNode("srvss_wp_driver", "srvss_wp_driver_node",name="srvss_wp_driver_node", args=arguments ,output="screen", respawn="false")
	self.launcher.launch(node)	
        time.sleep(3)
